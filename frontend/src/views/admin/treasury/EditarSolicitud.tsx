...
export default function EditarSolicitud() {
  ...
  const { getAllPayments, isLoading, getRequestById} = useTreasury();
  const currentYear = new Date().getFullYear();

  useEffect(() =>{
    const fecthRequestInfo= async () =>{
        const result: DataPostType['solicitudes']= await getRequestById(id)

        dispatch({type: "updateParams", payload: {name: "destacamento_id", value: result.id }})
        dispatch({type: "updateParams", payload: {name: "año", value: result.fecha_solicitud }})
